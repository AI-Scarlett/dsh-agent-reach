...
DSH 0.1.5-alpha.1/.2: the adapter uses only the public Skill filesystem provider contract, which is unchanged by the Agent, Inbox, Session V3, and Web panel updates. Source contract checks pass; Agent Reach runtime, channels, credentials and real Profile acceptance remain separate gates.
